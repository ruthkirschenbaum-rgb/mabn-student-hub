const opportunities = [
  ...
  {id:5,date:'',day:'TBD',month:'OCT',title:'MABN Data Blitz',org:'MABN Student Representatives',type:['MABN Event'],place:'Science Building',format:'Date & details',amount:'TBD',urgent:false,url:'',calendarKind:'event'},
...
if(countdown){
  ...
  countdown.textContent=Math.max(0,Math.ceil((deadline-today)/86400000));
}

function render(){
  const q=search.value.trim().toLowerCase();
  const rows=opportunities.filter(o=>((active==='All'&&!o.trainingOnly)||o.type.includes(active))&&(!q||[o.title,o.org,o.place,o.type.join(' ')].join(' ').toLowerCase().includes(q)));
  list.innerHTML=rows.map(o=>`<article class="opportunity">
    <div class="date"><small>${o.month}</small>${o.day}</div>
    <div class="main-info"><h3>${o.title}</h3><span class="tag ${o.urgent?'urgent':''}">${o.urgent?'Closing soon':o.type[0]}</span></div>
    <div class="meta"><b>${o.org}</b>${o.place}</div>
    <div class="meta"><b>${o.format}</b>${o.amount}</div>
    <div class="actions">${o.date?`<button class="icon-btn" data-cal="${o.id}" aria-label="Add to calendar">＋</button>`:''}${o.url?`<a class="go" href="${o.url}" target="_blank" rel="noreferrer" aria-label="Visit official opportunity page">↗</a>`:''}</div>
  </article>`).join('');
